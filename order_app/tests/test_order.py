...
class OrderTestBase(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.admin_user = User.objects.create_user(username='admin', email='admin@test.com', password='TestPass123!', type='business', is_staff=True)
        ...
        cls.offer = Offer.objects.create(user=cls.business_user1, title='Test Offer', description='Test')
        cls.offer_detail = OfferDetail.objects.create(offer=cls.offer, title='Basic Package', revisions=3, delivery_time_in_days=5, price=Decimal('150.00'), features=['Logo', 'Card'], offer_type='basic')
...
class OrderListTests(OrderTestBase):
    ...
    def setUpTestData(cls):
        super().setUpTestData()
        cls.order1 = Order.objects.create(customer_user=cls.customer_user, business_user=cls.business_user1, title='Order 1', revisions=3, delivery_time_in_days=5, price=Decimal('150.00'), features=['Logo'], offer_type='basic', status='in_progress')
        cls.order2 = Order.objects.create(customer_user=cls.customer_user, business_user=cls.business_user1, title='Order 2', revisions=5, delivery_time_in_days=10, price=Decimal('300.00'), features=['Website'], offer_type='premium', status='completed')
        cls.order3 = Order.objects.create(customer_user=cls.customer_user, business_user=cls.business_user2, title='Order 3', revisions=2, delivery_time_in_days=3, price=Decimal('100.00'), features=['App'], offer_type='standard', status='in_progress')
...
class OrderUpdateTests(OrderTestBase):
    ...
    def setUpTestData(cls):
        super().setUpTestData()
        cls.order = Order.objects.create(customer_user=cls.customer_user, business_user=cls.business_user1, title='Test Order', revisions=3, delivery_time_in_days=5, price=Decimal('150.00'), features=['Feature'], offer_type='basic', status='in_progress')
...
class OrderDeleteTests(OrderTestBase):
    ...
    def test_delete_success_as_admin(self):
        ...
        order = Order.objects.create(customer_user=self.customer_user, business_user=self.business_user1, title='Delete Me', revisions=3, delivery_time_in_days=5, price=Decimal('150.00'), features=['Feature'], offer_type='basic', status='in_progress')
    ...
    def test_delete_customer_forbidden(self):
        ...
        order = Order.objects.create(customer_user=self.customer_user, business_user=self.business_user1, title='Test', revisions=3, delivery_time_in_days=5, price=Decimal('150.00'), features=['Feature'], offer_type='basic', status='in_progress')
        self.client.credentials(HTTP_AUTHORIZATION=f'Token {self.customer_token.key}')
        url = reverse('order-detail', kwargs={'id': order.id})
        response = self.client.delete(url)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
...
class OrderCountTests(OrderTestBase):
    """Tests for GET /api/order-count/{business_user_id}/ - Count in_progress orders."""
    
    @classmethod
    def setUpTestData(cls):
        super().setUpTestData()
        for i in range(3):
            Order.objects.create(customer_user=cls.customer_user, business_user=cls.business_user1, title=f'Order {i}', revisions=3, delivery_time_in_days=5, price=Decimal('150.00'), features=['F'], offer_type='basic', status='in_progress')
        for i in range(2):
            Order.objects.create(customer_user=cls.customer_user, business_user=cls.business_user1, title=f'Done {i}', revisions=3, delivery_time_in_days=5, price=Decimal('150.00'), features=['F'], offer_type='basic', status='completed')
...
class CompletedOrderCountTests(OrderTestBase):
    """Tests for GET /api/completed-order-count/{business_user_id}/ - Count completed orders."""
    ...
    def setUpTestData(cls):
        ...
        for i in range(2):
            Order.objects.create(customer_user=cls.customer_user, business_user=cls.business_user1, title=f'Done {i}', revisions=3, delivery_time_in_days=5, price=Decimal('150.00'), features=['F'], offer_type='basic', status='completed')
        for i in range(3):
            Order.objects.create(customer_user=cls.customer_user, business_user=cls.business_user1, title=f'WIP {i}', revisions=3, delivery_time_in_days=5, price=Decimal('150.00'), features=['F'], offer_type='basic', status='in_progress')
...
class OrderSerializerTests(OrderTestBase):
    ...
    def test_features_list_serialization(self):
        ...
        order = Order.objects.create(customer_user=self.customer_user, business_user=self.business_user1, title='Test', revisions=3, delivery_time_in_days=5, price=Decimal('150.00'), features=['A', 'B', 'C'], offer_type='basic', status='in_progress')
        self.client.credentials(HTTP_AUTHORIZATION=f'Token {self.customer_token.key}')
        response = self.client.get(reverse('order-list'))
    ...
    def test_features_string_parsing(self):
        ...
        order = Order.objects.create(customer_user=self.customer_user, business_user=self.business_user1, title='Test', revisions=3, delivery_time_in_days=5, price=Decimal('150.00'), features='["X", "Y"]', offer_type='basic', status='in_progress')
        self.client.credentials(HTTP_AUTHORIZATION=f'Token {self.customer_token.key}')
        response = self.client.get(reverse('order-list'))
        order_data = [o for o in response.data if o['id'] == order.id][0]
        self.assertIsInstance(order_data['features'], list)
...
class OrderPermissionTests(OrderTestBase):
    ...
    def test_is_business_user_of_order_permission_success(self):
        ...
        order = Order.objects.create(customer_user=self.customer_user, business_user=self.business_user1, title='Test', revisions=3, delivery_time_in_days=5, price=Decimal('150.00'), features=['F'], offer_type='basic', status='in_progress')
        self.client.credentials(HTTP_AUTHORIZATION=f'Token {self.business_token1.key}')
        url = reverse('order-detail', kwargs={'id': order.id})
...
class OrderSerializerEdgeCaseTests(OrderTestBase):
    ...
    def test_features_nested_json_string(self):
        ...
        order = Order.objects.create(customer_user=self.customer_user, business_user=self.business_user1, title='Nested JSON', revisions=3, delivery_time_in_days=5, price=Decimal('150.00'), features='["Item1", "Item2"]', offer_type='basic', status='in_progress')
        self.client.credentials(HTTP_AUTHORIZATION=f'Token {self.customer_token.key}')
        response = self.client.get(reverse('order-list'))
        order_data = [o for o in response.data if o['id'] == order.id][0]
    ...
    def test_features_malformed_json_fallback(self):
        ...
        order = Order.objects.create(customer_user=self.customer_user, business_user=self.business_user1, title='Malformed', revisions=3, delivery_time_in_days=5, price=Decimal('150.00'), features='invalid json {[', offer_type='basic', status='in_progress')
        self.client.credentials(HTTP_AUTHORIZATION=f'Token {self.customer_token.key}')
        response = self.client.get(reverse('order-list'))
        order_data = [o for o in response.data if o['id'] == order.id][0]
        self.assertIsInstance(order_data['features'], list)
    ...
    def test_features_ast_literal_eval_path(self):
        ...
        order = Order.objects.create(customer_user=self.customer_user, business_user=self.business_user1, title='AST Parse', revisions=3, delivery_time_in_days=5, price=Decimal('150.00'), features="['Feature A', 'Feature B']", offer_type='basic', status='in_progress')
        self.client.credentials(HTTP_AUTHORIZATION=f'Token {self.customer_token.key}')
        response = self.client.get(reverse('order-list'))
        order_data = [o for o in response.data if o['id'] == order.id][0]
    ...
    def test_features_double_json_encoded(self):
        ...
        order = Order.objects.create(customer_user=self.customer_user, business_user=self.business_user1, title='Double Encoded', revisions=3, delivery_time_in_days=5, price=Decimal('150.00'), features='"[\\"A\\", \\"B\\"]"', offer_type='basic', status='in_progress')
        self.client.credentials(HTTP_AUTHORIZATION=f'Token {self.customer_token.key}')
        response = self.client.get(reverse('order-list'))
        order_data = [o for o in response.data if o['id'] == order.id][0]
        self.assertIsInstance(order_data['features'], list)
    ...
    def test_features_single_value_becomes_list(self):
        ...
        order = Order.objects.create(customer_user=self.customer_user, business_user=self.business_user1, title='Single Value', revisions=3, delivery_time_in_days=5, price=Decimal('150.00'), features='123', offer_type='basic', status='in_progress')
        self.client.credentials(HTTP_AUTHORIZATION=f'Token {self.customer_token.key}')
        response = self.client.get(reverse('order-list'))
        order_data = [o for o in response.data if o['id'] == order.id][0]
        self.assertIsInstance(order_data['features'], list)
    ...
    def test_features_empty_list(self):
        ...
        order = Order.objects.create(customer_user=self.customer_user, business_user=self.business_user1, title='Empty Features', revisions=3, delivery_time_in_days=5, price=Decimal('150.00'), features=[], offer_type='basic', status='in_progress')
        self.client.credentials(HTTP_AUTHORIZATION=f'Token {self.customer_token.key}')
        response = self.client.get(reverse('order-list'))
        order_data = [o for o in response.data if o['id'] == order.id][0]
    ...
    def test_features_integer_value_becomes_list(self):
        ...
        order = Order.objects.create(customer_user=self.customer_user, business_user=self.business_user1, title='Integer Features', revisions=3, delivery_time_in_days=5, price=Decimal('150.00'), features=456, offer_type='basic', status='in_progress')
        self.client.credentials(HTTP_AUTHORIZATION=f'Token {self.customer_token.key}')
        response = self.client.get(reverse('order-list'))
        order_data = [o for o in response.data if o['id'] == order.id][0]
        self.assertIsInstance(order_data['features'], list)
    ...
    def test_update_serializer_features_parsing(self):
        ...
        order = Order.objects.create(customer_user=self.customer_user, business_user=self.business_user1, title='Update Test', revisions=3, delivery_time_in_days=5, price=Decimal('150.00'), features='["Old Feature"]', offer_type='basic', status='in_progress')
        self.client.credentials(HTTP_AUTHORIZATION=f'Token {self.business_token1.key}')
        url = reverse('order-detail', kwargs={'id': order.id})
        response = self.client.patch(url, {'status': 'completed'}, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_validate_status_allowed_values(self):
        ...
        order = Order.objects.create(customer_user=self.customer_user, business_user=self.business_user1, title='Status Test', revisions=3, delivery_time_in_days=5, price=Decimal('150.00'), features=['F'], offer_type='basic', status='in_progress')
        self.client.credentials(HTTP_AUTHORIZATION=f'Token {self.business_token1.key}')
        url = reverse('order-detail', kwargs={'id': order.id})
...
class OrderPermissionEdgeCaseTests(OrderTestBase):
    ...
    def test_is_business_user_permission_no_type_attribute(self):
        ...
        order = Order.objects.create(customer_user=self.customer_user, business_user=self.business_user1, title='Permission Test', revisions=3, delivery_time_in_days=5, price=Decimal('150.00'), features=['F'], offer_type='basic', status='in_progress')
        ...
        has_permission = permission.has_object_permission(mock_request, None, order)
        self.assertFalse(has_permission)
    
    def test_is_business_user_permission_customer_type(self):
        """Test permission explicitly denies customer type users."""
        order = Order.objects.create(customer_user=self.customer_user, business_user=self.business_user1, title='Customer Type Test', revisions=3, delivery_time_in_days=5, price=Decimal('150.00'), features=['F'], offer_type='basic', status='in_progress')
        from order_app.api.permissions import IsBusinessUserOfOrder
        permission = IsBusinessUserOfOrder()
...
class OrderViewExceptionTests(OrderTestBase):
    ...
    def test_create_view_exception_handling(self):
        ...
        self.client.credentials(HTTP_AUTHORIZATION=f'Token {self.customer_token.key}')
        response = self.client.post(reverse('order-list'), {'offer_detail_id': self.offer_detail.id}, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
...
class OrderModelTests(OrderTestBase):
    ...
    def test_order_str_method(self):
        ...
        order = Order.objects.create(customer_user=self.customer_user, business_user=self.business_user1, title='Test Order', revisions=3, delivery_time_in_days=5, price=Decimal('150.00'), features=['Feature'], offer_type='basic', status='in_progress')
    ...
    def test_order_default_values(self):
        ...
        order = Order.objects.create(customer_user=self.customer_user, business_user=self.business_user1, title='Defaults Test', revisions=0, delivery_time_in_days=5, price=Decimal('100.00'), offer_type='basic')
    ...
    def test_order_status_choices(self):
        ...
        for status_choice, _ in Order.STATUS_CHOICES:
            order = Order.objects.create(customer_user=self.customer_user, business_user=self.business_user1, title=f'Status {status_choice}', revisions=3, delivery_time_in_days=5, price=Decimal('150.00'), features=['F'], offer_type='basic', status=status_choice)
    ...
    def test_order_offer_type_choices(self):
        ...
        for offer_type, _ in Order.OFFER_TYPE_CHOICES:
            order = Order.objects.create(customer_user=self.customer_user, business_user=self.business_user1, title=f'Type {offer_type}', revisions=3, delivery_time_in_days=5, price=Decimal('150.00'), features=['F'], offer_type=offer_type, status='in_progress')
