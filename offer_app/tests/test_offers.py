...
class OfferDetailViewTests(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.detail3 = OfferDetail.objects.create(
            offer=cls.offer,
    ...
    def test_get_offer_detail_success(self):
        ...
        response = self.client.get(
            reverse('offer-detail', kwargs={'id': self.offer.id})
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_get_offer_detail_not_found(self):
        ...
        response = self.client.get(
            reverse('offer-detail', kwargs={'id': 99999})
        )
...
class OfferUpdateTests(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.other_business = User.objects.create_user(
            username='business2',
            email='business2@test.com',
            password='TestPass123!',
            type='business'
        )
        cls.other_business_token = Token.objects.create(
            user=cls.other_business
        )
    ...
    def test_update_offer_user_field_ignored(self):
        ...
        response = self.client.patch(
            reverse('offer-detail', kwargs={'id': self.offer.id}),
            {'title': 'Updated Title', 'user': self.other_business.id},
            format='json'
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class OfferDeleteTests(APITestCase):
    ...
    def test_delete_offer_with_multiple_details(self):
        ...
        self.assertEqual(
            OfferDetail.objects.filter(offer=self.offer).count(),
            3
        )
        self.client.credentials(
            HTTP_AUTHORIZATION=f'Token {self.business_token.key}'
        ...
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
        self.assertEqual(
            OfferDetail.objects.filter(offer_id=self.offer.id).count(),
            0
        )


class OfferDetailSingleTests(APITestCase):
    """Tests for single offer detail retrieval."""
    ...
    def test_get_offerdetail_success(self):
        ...
        response = self.client.get(
            reverse('offerdetail-single', kwargs={'id': self.detail.id})
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_get_offerdetail_not_found(self):
        ...
        response = self.client.get(
            reverse('offerdetail-single', kwargs={'id': 99999})
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
...
class OfferModelTests(APITestCase):
    ...
    def test_offer_min_price_with_details(self):
        ...
        offer = Offer.objects.create(
            ...
            description='Various design services'
        )
        OfferDetail.objects.create(
            offer=offer,
            title='Basic',
    ...
    def test_offer_min_price_updates_after_detail_added(self):
        ...
        offer = Offer.objects.create(
            user=self.business_user,
            title='Evolving Offer',
            description='Prices change'
        )
        OfferDetail.objects.create(
            offer=offer,
            title='Expensive',
    ...
    def test_offer_min_delivery_time_updates_after_detail_added(self):
        ...
        offer = Offer.objects.create(
            user=self.business_user,
            title='Evolving Offer',
            description='Delivery times change'
        )
        OfferDetail.objects.create(
            offer=offer,
            title='Slow',
