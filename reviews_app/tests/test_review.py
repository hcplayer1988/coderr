...
class ReviewAPITestCase(APITestCase):
    """Base class with common setup for all review tests."""
    ...
    def setUpTestData(cls):
        ...
        cls.reviewer2 = User.objects.create_user(
            username='reviewer2',
...
class ReviewListTests(ReviewAPITestCase):
    """Tests for GET /api/reviews/ - List reviews."""
    ...
    def test_list_reviews_filter_by_business_user(self):
        ...
        url = (
            f"{reverse('review-list')}"
            f"?business_user_id={self.business_user1.id}"
        )
...
class ReviewCreateTests(ReviewAPITestCase):
    """Tests for POST /api/reviews/ - Create review."""
    ...
    def test_create_review_duplicate_not_allowed(self):
        ...
        data = {
            'business_user': self.business_user1.id,
            'rating': 5,
            'description': 'Trying to add another review'
        }
        response = self.client.post(
            reverse('review-list'),
            data,
...
class ReviewUpdateTests(ReviewAPITestCase):
    """Tests for PATCH /api/reviews/{id}/ - Update review."""
    ...
    def test_update_review_not_owner(self):
        self.client.credentials(
            HTTP_AUTHORIZATION=f'Token {self.reviewer2_token.key}'
        )
        url = reverse('review-detail', kwargs={'id': self.review.id})
        data = {'rating': 1}
        response = self.client.patch(url, data, format='json')
...
class ReviewDeleteTests(ReviewAPITestCase):
    """Tests for DELETE /api/reviews/{id}/ - Delete review."""
