...
class OrderAPITestCase(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.offer = Offer.objects.create(user=cls.business_user1, title='Test Offer', description='Test Description')
        cls.offer_detail = OfferDetail.objects.create(offer=cls.offer, title='Basic Package', revisions=3, delivery_time_in_days=5, price=Decimal('150.00'), features=['Logo Design', 'Visitenkarten'], offer_type='basic')
...
class OrderListTests(OrderAPITestCase):
    ...
    def setUpTestData(cls):
        super().setUpTestData()
        cls.order1 = Order.objects.create(customer_user=cls.customer_user, business_user=cls.business_user1, title='Logo Design', revisions=3, delivery_time_in_days=5, price=Decimal('150.00'), features=['Logo Design', 'Visitenkarten'], offer_type='basic', status='in_progress')
        cls.order2 = Order.objects.create(customer_user=cls.customer_user, business_user=cls.business_user1, title='Website Development', revisions=5, delivery_time_in_days=14, price=Decimal('500.00'), features=['Website', 'SEO'], offer_type='premium', status='completed')
        cls.order3 = Order.objects.create(customer_user=cls.customer_user, business_user=cls.business_user2, title='Mobile App', revisions=10, delivery_time_in_days=30, price=Decimal('2000.00'), features=['iOS App', 'Android App'], offer_type='premium', status='in_progress')
...
class OrderUpdateTests(OrderAPITestCase):
    ...
    def test_update_read_only_fields(self):
        self.client.credentials(HTTP_AUTHORIZATION=f'Token {self.business_token1.key}')
        url = reverse('order-detail', kwargs={'id': self.order.id})
        data = {'status': 'completed', 'title': 'Changed Title', 'price': '999.99'}
        response = self.client.patch(url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['status'], 'completed')
...
class OrderDeleteTests(OrderAPITestCase):
    ...
    def test_delete_success_as_admin(self):
        order = Order.objects.create(customer_user=self.customer_user, business_user=self.business_user1, title='To Delete', revisions=3, delivery_time_in_days=5, price=Decimal('150.00'), features=['Feature'], offer_type='basic', status='in_progress')
...
class OrderCountTests(OrderAPITestCase):
    ...
    def setUpTestData(cls):
        super().setUpTestData()
        for i in range(3):
            Order.objects.create(customer_user=cls.customer_user, business_user=cls.business_user1, title=f'Order {i+1}', revisions=3, delivery_time_in_days=5, price=Decimal('150.00'), features=['Feature'], offer_type='basic', status='in_progress')
        for i in range(2):
            Order.objects.create(customer_user=cls.customer_user, business_user=cls.business_user1, title=f'Completed {i+1}', revisions=3, delivery_time_in_days=5, price=Decimal('150.00'), features=['Feature'], offer_type='basic', status='completed')
...
class CompletedOrderCountTests(OrderAPITestCase):
    ...
    def setUpTestData(cls):
        super().setUpTestData()
        for i in range(2):
            Order.objects.create(customer_user=cls.customer_user, business_user=cls.business_user1, title=f'Completed Order {i+1}', revisions=3, delivery_time_in_days=5, price=Decimal('150.00'), features=['Feature'], offer_type='basic', status='completed')
        for i in range(3):
            Order.objects.create(customer_user=cls.customer_user, business_user=cls.business_user1, title=f'In Progress {i+1}', revisions=3, delivery_time_in_days=5, price=Decimal('150.00'), features=['Feature'], offer_type='basic', status='in_progress')
